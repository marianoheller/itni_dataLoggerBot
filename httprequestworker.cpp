...
void HttpRequestInput::setContentType(QString type) {
    contentType = type;
}

void HttpRequestInput::setPayload(QByteArray pl) {
    payload = pl;
}
...
void HttpRequestWorker::execute(HttpRequestInput *input) {
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        if (!input->payload.isEmpty())
            request_content.append(input->payload);
        ...
        foreach (QString key, input->vars.keys()) {
            ...
            request_content.append("Content-Type: "+ input->contentType);
            request_content.append(new_line);
    ...
    // prepare connection
    requestHeader.data = request_content;
...
void HttpRequestWorker::on_manager_finished(QNetworkReply *reply) {
    ...
    if (request.hasRawHeader("Content-Type") ) {
        requestHeader.contentType = request.header(QNetworkRequest::ContentTypeHeader);
    }
    if (request.hasRawHeader("User-Agent")) {
        requestHeader.userAgent = request.rawHeader("User-Agent");
    }
    //PARSE RESPONSE
